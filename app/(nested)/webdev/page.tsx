	...
	return (
		<div className={`${styles.root} min-w-0 min-h-0 flex flex-col`}>
			<div className={styles.square}>
				<AILogo className="w-full h-full object-contain text-opacity-40 text-slate-100"/>
				...
				>
					<AILogo className="text-opacity-40 text-slate-100" />
				</div>
				<div
				...
				<div 
					...
					style={{ '--compensation': `${compensation}` }}
				></div>
				<div className={`${styles.center} absolute w-28 h-28 lg:w-32 lg:h-32 border-[length:var(--border-thickness)] border-white rounded-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2`}></div>
			</div>
			...
			<div className="relative md:min-w-96 mt-40 md:mt-0 md:mr-[calc(var(--length)+16px)] flex-1 bg-slate-700 bg-opacity-90 rounded group-[:not(.open)]/client:opacity-0 transition-all duration-[var(--web-time)] delay-[var(--web-time)] z-20">
				<div className={`${styles.from_top} ${styles.content}`} ref={(el) => { contentRefs.current[0] = el; }}>
					Experience content
				</div>
				<div className={styles.content} ref={(el) => { contentRefs.current[1] = el; }}>
					Skills content
				</div>
				<div className={styles.content} ref={(el) => { contentRefs.current[2] = el; }}>
					Projects content
				</div>
			</div>
		</div>
